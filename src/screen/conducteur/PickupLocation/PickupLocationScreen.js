// screen/conducteur/PickupLocation/PickupLocationScreen.js
...
const PickupLocationScreen = () => {
  ...
  // ⚠️  departure est une string
  const departureCity = typeof departure === 'string' ? departure : (departure?.city || '');
  ...
  const nearbySearch = useCallback(async ({ lat, lng }, type) => {
    ...
    const url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=${radius}&type=${type}&language=fr&key=${API_KEY}`;
    const res = await fetch(url);
    const data = await res.json();
  ...
  const fetchSuggestionsNear = useCallback(async (center, cityLabel) => {
    if (!API_KEY) {
    ...
    try {
      ...
      console.warn('fetchSuggestionsNear error:', e?.message || e);
      setPublicSuggestions(COMMON_FALLBACKS(cityLabel || ''));
      setErrorSuggestions('Impossible de charger les suggestions, fallback proposé.');
    } finally {
  ...
  const fetchSuggestionsForCity = useCallback(async (city) => {
    ...
    try {
      ...
      await fetchSuggestionsNear(center, city);
    } catch (e) {
      console.warn('fetchSuggestionsForCity error:', e?.message || e);
      setPublicSuggestions(COMMON_FALLBACKS(city));
      setErrorSuggestions('Erreur de recherche, fallback proposé.');
    } finally {
      setLoadingSuggestions(false);
    }
  }, [geocodeCity, fetchSuggestionsNear]);

  useEffect(() => {
    if (departureCity && !usingMyLocation) {
      fetchSuggestionsForCity(departureCity);
      // Préremplir léger
      setSearchText('');
    }
  }, [departureCity, usingMyLocation, fetchSuggestionsForCity]);

  // ---------------- Sélection d’un lieu (met aussi le texte dans l’input) ----------------
  const selectPlace = (item) => {
    ...
    locationRef.current = {
      ...
      city: item.city || departureCity || currentCityName,
    ...
    const display = item.address && item.address !== item.name
  ...
  const useMyLocation = async () => {
    try {
      ...
      // On met un hint dans l'input mais ce n'est pas encore un lieu sélectionné
      setSearchText(`Autour de : ${cityLabel}`);

      await fetchSuggestionsNear(coords, cityLabel);
      setUsingMyLocation(true);
      setIsValid(false); // l’utilisateur doit encore choisir un lieu précis
  ...
  const handleShortcut = async (kind) => {
    ...
    try {
      ...
      // ✅ sélection + met le texte
      selectPlace(sorted[0]);
    } catch (e) {
  ...
  const handleConfirm = () => {
    ...
    navigation.navigate('DropoffLocation', {
      ...
      time,
      stops,
      pickupLocation: locationRef.current || searchText,
    });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Champ de recherche (contrôlé) */}
      <GooglePlacesInputPublicOnly
        key={searchText}                // 👈 force le remount si le compo ignore 'value'
        placeholder="Rechercher un lieu public"
        baseLocation={usingMyLocation ? (currentCityName || departureCity) : departureCity}
        value={searchText}              // 👈 si ton compo supporte 'value'
        initialValue={searchText}       // 👈 sinon il prendra cette valeur au mount
        onChangeText={setSearchText}    // 👈 si ton compo relaie les changements
        onSelect={(place) => {
          const item = {
            place_id: place.place_id || place.id,
            name: place.name || place.title || place.description,
            address: place.formatted_address || place.address || place.vicinity || (usingMyLocation ? currentCityName : departureCity),
            location: {
              latitude: place.latitude || place?.geometry?.location?.lat,
              longitude: place.longitude || place?.geometry?.location?.lng,
            },
            city: usingMyLocation ? currentCityName : departureCity,
            types: place.types || [],
          };
